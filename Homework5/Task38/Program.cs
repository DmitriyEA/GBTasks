...
WriteLine($"Разница между значениями max и min элементов, равна: {SubtractionResult(array)}");
...
{
    int result = 0;
    if (inArray.Length < 2) return result;
    ...
    for (int i = 2; i < inArray.Length; i++)
    {
        ...
        else if (inArray[i] > max) max = inArray[i];
    }
